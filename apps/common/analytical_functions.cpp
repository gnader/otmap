...
double circles(const Eigen::Vector2d &p, bool boundary)
{
    ...
    if (boundary && (l1 > 0.5 - r2))
        return 1;
    if ((p - Vector2d(t1, t1)).norm() < r1 || (p - Vector2d(t1, 1. - t1)).norm() < r1 || (p - Vector2d(1. - t1, 1. - t1)).norm() < r1 || (p - Vector2d(1. - t1, t1)).norm() < r1)
    {
        return 1;
    }
    return 0;
...
double BFO12_example(const Eigen::Vector2d &p)
{
    ...
    double qx = q(pt.x());
    double dqx = dq(pt.x());
    double d2qx = d2q(pt.x());
    double qy = q(pt.y());
    double dqy = dq(pt.y());
    double d2qy = d2q(pt.y());
...
double func(FuncName::Enum fn, const Eigen::Vector2d &p)
{
    ...
    case BFO12EX:
        ...
        break;
    case BFO12ELLIPSE1:
        val = BFO12_ellipse1(p);
        break;
    case BFO12ELLIPSE2:
        val = BFO12_ellipse2(p);
        break;
    case BFO12_1GAUSS:
        val = BFO12_1gauss(p);
        break;
    ...
    case DCFCL08EX1:
        val = DCFCL08_example1(p);
        break;
    case DCFCL08EX2:
        val = DCFCL08_example2(p);
        break;
    case DCFCL08EX3:
        val = DCFCL08_example3(p);
        break;
