    ...
    transport_cost(const Surface_mesh &src_mesh, const Surface_mesh &dst_mesh, const VectorXd &density_per_face, VectorXd *cost_per_face)
    {
        ...
        for (int i = 0; i < nfaces; ++i)
        {
            ...
            if (nfaces == 2 * density_per_face.size())
            {
                ...
                {
                    return ((1 - u) * (1 - v) * (dst_mesh.position(v1) - src_mesh.position(v1)) + u * (1 - v) * (dst_mesh.position(v2) - src_mesh.position(v2)) + u * v * (dst_mesh.position(v3) - src_mesh.position(v3)) + (1 - u) * v * (dst_mesh.position(v4) - src_mesh.position(v4))).squaredNorm();
                };
            ...
            else
            {
                ...
                {
                    return (l1 * (dst_mesh.position(v1) - src_mesh.position(v1)) + l2 * (dst_mesh.position(v2) - src_mesh.position(v2)) + l3 * (dst_mesh.position(v3) - src_mesh.position(v3))).squaredNorm();
                };
