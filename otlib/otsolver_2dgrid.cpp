...
namespace otmap
{
    ...
    GridBasedTransportSolver::solve(ConstRefVector in_density, SolverOptions opt)
    {
        if (m_verbose_level >= 1)
        {
            ...
            if (opt.beta == BetaOpt::Zero)
                std::cout << "0";
            if (opt.beta == BetaOpt::ConjugateJacobian)
                std::cout << "Conjugate-Jacobian";
        ...
        while (it < opt.max_iter && residual > opt.threshold)
        {
            ...
            timer.stop();
            t_linearsolve = timer.value(REAL_TIMER);
            t_linearsolve_sum += t_linearsolve;
            timer.start();
            ...
            timer.stop();
            t_linesearch = timer.value(REAL_TIMER);
            t_linesearch_sum += t_linesearch;
        ...
        initialize_laplacian_solver()
    {
        ...
        timer.start();
        {
            ...
            if (m_laplacian_solver.info() != Success)
            {
                ...
                if (m_laplacian_solver.info() == Success)
                    std::cout << "Success\n";
                else if (m_laplacian_solver.info() == NumericalIssue)
                    std::cout << "NumericalIssue\n";
                else if (m_laplacian_solver.info() == NoConvergence)
                    std::cout << "NoConvergence\n";
                else if (m_laplacian_solver.info() == InvalidInput)
    ...
    void compute_face_area(VectorXd &fwd_area, const MatrixX2d &vtx_grads, int grid_size)
    {
        ...
        for (int i = 0; i < grid_size; ++i)
        {
            ...
            for (int j = 0; j < grid_size; ++j)
            {
                ...
                fwd_area(id) = 0.5 * ((vtx_grads(v11, 0) - vtx_grads(v00, 0) + e) * (vtx_grads(v01, 1) - vtx_grads(v10, 1) + e) - (vtx_grads(v11, 1) - vtx_grads(v00, 1) + e) * (vtx_grads(v01, 0) - vtx_grads(v10, 0) - e));
            }
        }
    }
        ...
        compute_transport_cost(const MatrixX2d &vtx_grads, VectorXd &cost) const
    {
        ...
        for (int i = 0; i < m_gridSize; ++i)
        {
            for (int j = 0; j < m_gridSize; ++j)
            {
                ...
                {
                    return ((1 - u) * (1 - v) * vtx_grads.row(v1) + u * (1 - v) * vtx_grads.row(v2) + u * v * vtx_grads.row(v3) + (1 - u) * v * vtx_grads.row(v4)).squaredNorm();
                };
